	...
	while (WindowShouldClose() != true) {
		...
		for (int row = 0; row < 32; row++)
			for (int col = 0; col < 32; col++) {
				...
				if (tile_pos < 128)
					DrawTexturePro(tile_data_block_1, (Rectangle){ .x = 0, .y = 8 * tile_pos, .width = 8, .height = 8 }, (Rectangle){ .x = col * 16, .y = row * 16, .width = 16, .height = 16}, (Vector2){ 0, 0}, 0.0, WHITE);
				else
					DrawTexturePro(tile_data_block_2, (Rectangle){ .x = 0, .y = 8 * (tile_pos - 128), .width = 8, .height = 8 }, (Rectangle){ .x = col * 8, .y = row * 8, .width = 8, .height = 8}, (Vector2){ 0, 0}, 0.0, WHITE);
			}
		DrawFPS(10, 10);
		EndDrawing();
	}
